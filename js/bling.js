...
table.dump td.v.bool      { background-color: #fcf; }`));
  ...
  $.plugin({
    ...
    prior_date = 0;
    get_date_prefix = function() {
      var d, day, delta, hr, min, mon, ms, sec;
      d = new Date();
      delta = d - prior_date;
      prior_date = d;
      if ((ts.ms !== (ms = $.padLeft(d.getUTCMilliseconds(), 3, "0"))) || (delta % 1000) === 0) {
        ts.ms = ms;
        if (ts.SS !== (sec = $.padLeft(d.getUTCSeconds(), 2, "0" || (delta % 60000) === 0))) {
          ts.SS = sec;
          if (ts.MM !== (min = $.padLeft(d.getUTCMinutes(), 2, "0" || (delta % 3600000) === 0))) {
            ts.MM = min;
            if (ts.HH !== (hr = $.padLeft(String(d.getUTCHours()), 2, "0" || (delta % 86400000) === 0))) {
              ts.HH = hr;
              if (ts.dd !== (day = $.padLeft(String(d.getUTCDate()), 2, "0" || (delta > 86400000)))) {
                ts.dd = day;
                if (ts.mm !== (mon = $.padLeft(String(d.getUTCMonth() + 1), 2, "0"))) {
                  ts.mm = mon;
      ...
      ret = $.inherit({
        ...
        wait: function(timeout, cb) {
          ...
          if (err !== NoValue) {
            ...
            waiting.push(cb);
            if (isFinite(timeout = parseFloat(timeout)) && timeout > 0) {
  ...
  $.plugin({
    ...
    var defer, throttle;
    defer = function(f, ctx, args, ms, prior) {
      clearTimeout(prior);
      return setTimeout((() => {
 ...
 then fire one last time after a gap of _ms_ at the end.`;
  ...
  $.plugin({
    ...
    var __toString, _type, inherit, isType, max, maxHash, min;
    ({max, min} = Math);
    maxHash = 0xFFFFFFFF;
    ...
    _type.extend({
      ...
      number: {
        array: function(o) {
          return $.extend(new Array(max(0, min(o, maxHash))), {
            length: 0
          });
        }
      },
